...
import fund.ergoindex.backend.auth.RedisResource
...
object Program:
  ...
  def make(executionContext: ExecutionContext): IO[Unit] =
    RedisResource.make().use { redis =>
      for
        keyPairBoundary <- IO(
          KeyPairBoundary
            .make(BcEd25519KeyPairController.make(BcEd25519KeyPairEntityGateway.make()))
        )
        keyPairOrErr <- keyPairBoundary.getOrGenerate().value
        keyPair      <- keyPairOrErr.fold(throwKeyPairE, IO.apply)
        httpApp      <- DependencyGraph.make(keyPair.getPrivate, keyPair.getPublic, redis)
